...
const std::vector<Vertex> kVertices = {
    ...
    /* Near rectangle: submitted first. */
    {{-0.60F, -0.60F, 0.0F}, {1.0F, 1.0F, 1.0F}, {0.0F, 1.0F}},
    {{ 0.40F, -0.60F, 0.0F}, {1.0F, 1.0F, 1.0F}, {1.0F, 1.0F}},
    {{ 0.40F,  0.40F, 0.0F}, {1.0F, 1.0F, 1.0F}, {1.0F, 0.0F}},
    {{-0.60F,  0.40F, 0.0F}, {1.0F, 1.0F, 1.0F}, {0.0F, 0.0F}},
    /* Far rectangle: submitted second, but rejected where the near one covers it. */
    {{-0.30F, -0.30F, -0.35F}, {0.65F, 0.65F, 0.65F}, {0.0F, 1.0F}},
    {{ 0.70F, -0.30F, -0.35F}, {0.65F, 0.65F, 0.65F}, {1.0F, 1.0F}},
    {{ 0.70F,  0.70F, -0.35F}, {0.65F, 0.65F, 0.65F}, {1.0F, 0.0F}},
    {{-0.30F,  0.70F, -0.35F}, {0.65F, 0.65F, 0.65F}, {0.0F, 0.0F}},
};

constexpr uint32_t kTextureWidth = 4;
constexpr uint32_t kTextureHeight = 4;
/* It arranges the pixels {R, G, B, A} in a 4x4 grid as follows:
 * red     red     yellow  yellow
 * red     red     yellow  yellow
 * blue    blue    green   green
 * blue    blue    green   green
 */
constexpr std::array<uint8_t, kTextureWidth * kTextureHeight * 4> kTexturePixels = {
    255,  64,  64, 255, 255,  64,  64, 255, 255, 220,  64, 255, 255, 220,  64, 255,
    255,  64,  64, 255, 255,  64,  64, 255, 255, 220,  64, 255, 255, 220,  64, 255,
     64, 120, 255, 255,  64, 120, 255, 255,  64, 255, 180, 255,  64, 255, 180, 255,
     64, 120, 255, 255,  64, 120, 255, 255,  64, 255, 180, 255,  64, 255, 180, 255,
};
...
struct SwapChainContext {
    ...
    VkImage depthImage = VK_NULL_HANDLE;        // Depth image used for depth testing in the rendering pipeline.
    VkDeviceMemory depthImageMemory = VK_NULL_HANDLE;   // device-local memory allocated for the depth image, ensuring efficient access during rendering.
...
GraphicsPipelineContext createGraphicsPipeline(
    ...
    depthStencil.depthCompareOp = VK_COMPARE_OP_LESS;   // Lower depth values are closer to the camera.
...
VkFormat findDepthFormat(VkPhysicalDevice physicalDevice) {
    return findSupportedFormat(     // find a supported depth format. 
        physicalDevice,
...
void createDepthResources(
    ...
    swapChainContext.depthFormat = findDepthFormat(physicalDevice);
    ...
    imageInfo.format = swapChainContext.depthFormat;
    imageInfo.tiling = VK_IMAGE_TILING_OPTIMAL;
    imageInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
    imageInfo.usage = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;  // Usage flag for depth and stencil attachment
    imageInfo.samples = VK_SAMPLE_COUNT_1_BIT;
    imageInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;

    if (vkCreateImage(device, &imageInfo, nullptr, &swapChainContext.depthImage) !=
        VK_SUCCESS) {
        throw std::runtime_error("Failed to create depth image.");
    }

    VkMemoryRequirements memoryRequirements{};
    vkGetImageMemoryRequirements(device, swapChainContext.depthImage, &memoryRequirements);
    ...
    if (vkAllocateMemory(
            ...
            &allocateInfo,
            nullptr,
            &swapChainContext.depthImageMemory
        ) != VK_SUCCESS) {
        vkDestroyImage(device, swapChainContext.depthImage, nullptr);
    ...
    if (vkBindImageMemory(
        ...
        vkDestroyImage(device, swapChainContext.depthImage, nullptr);
        vkFreeMemory(device, swapChainContext.depthImageMemory, nullptr);
        throw std::runtime_error("Failed to bind depth image memory.");
    }

    swapChainContext.depthImageView = createImageView(
        device,
        swapChainContext.depthImage,
...
void destroyBuffer(VkDevice device, const VertexBufferContext& bufferContext) {
    if (bufferContext.buffer != VK_NULL_HANDLE) {
        vkDestroyBuffer(device, bufferContext.buffer, nullptr);
    }

    if (bufferContext.memory != VK_NULL_HANDLE) {
        vkFreeMemory(device, bufferContext.memory, nullptr);
    }
}
...
void transitionImageLayout(
    ...
    vkCmdPipelineBarrier(   // here is the actual layout transition command, which will be executed when the command buffer is submitted.
        commandBuffer,
        sourceStage,
        destinationStage,
        0,
...
VertexBufferContext createVertexBuffer(
    ...
    createBuffer(
        ...
        deviceContext.device,
        bufferSize,
        VK_BUFFER_USAGE_TRANSFER_SRC_BIT,   // source of GPU transfer, so we can copy it to the vertex buffer.
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, // CPU-visible memory, so we can map it and write to it directly.
        stagingBuffer.buffer,
        stagingBuffer.memory
    );

    VertexBufferContext vertexBuffer{};
    createBuffer(
        physicalDevice,
        deviceContext.device,
        bufferSize,
    ...
    try {
        ...
        destroyBuffer(deviceContext.device, vertexBuffer);
        throw;
    }
...
TextureContext createTexture(
    VkPhysicalDevice physicalDevice,
    const DeviceContext& deviceContext,
    VkSurfaceKHR surface
) {
    const VkDeviceSize imageSize = kTexturePixels.size();
    VertexBufferContext stagingBuffer{};
    createBuffer(
        physicalDevice,
        deviceContext.device,
    ...
    void* data = nullptr;
    if (vkMapMemory(deviceContext.device, stagingBuffer.memory, 0, imageSize, 0, &data) !=
        VK_SUCCESS) {
        destroyBuffer(deviceContext.device, stagingBuffer);
        throw std::runtime_error("Failed to map texture staging memory.");
    }
    std::memcpy(data, kTexturePixels.data(), kTexturePixels.size());
    vkUnmapMemory(deviceContext.device, stagingBuffer.memory);
    ...
    if (vkCreateImage(deviceContext.device, &imageInfo, nullptr, &texture.image) != VK_SUCCESS) {
        destroyBuffer(deviceContext.device, stagingBuffer);
        throw std::runtime_error("Failed to create texture image.");
    }

    VkMemoryRequirements memoryRequirements{};
    vkGetImageMemoryRequirements(deviceContext.device, texture.image, &memoryRequirements);
    VkMemoryAllocateInfo allocateInfo{};
    allocateInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    allocateInfo.allocationSize = memoryRequirements.size;
    allocateInfo.memoryTypeIndex = findMemoryType(
    ...
    if (vkAllocateMemory(deviceContext.device, &allocateInfo, nullptr, &texture.memory) !=
        VK_SUCCESS ||
        vkBindImageMemory(deviceContext.device, texture.image, texture.memory, 0) != VK_SUCCESS) {
        vkDestroyImage(deviceContext.device, texture.image, nullptr);
        if (texture.memory != VK_NULL_HANDLE) {
            vkFreeMemory(deviceContext.device, texture.memory, nullptr);
        }
        destroyBuffer(deviceContext.device, stagingBuffer);
    ...
    if (vkCreateCommandPool(deviceContext.device, &poolInfo, nullptr, &commandPool) != VK_SUCCESS) {
        ...
        vkFreeMemory(deviceContext.device, texture.memory, nullptr);
        destroyBuffer(deviceContext.device, stagingBuffer);
        throw std::runtime_error("Failed to create texture transfer command pool.");
    }
    ...
    samplerInfo.magFilter = VK_FILTER_NEAREST;  // Nearest-neighbor filtering for magnification.
...
void destroyTexture(VkDevice device, const TextureContext& texture) {
...
void destroySwapChainResources(
    ...
    if (swapChainContext.depthImageView != VK_NULL_HANDLE) {
        vkDestroyImageView(device, swapChainContext.depthImageView, nullptr);
        swapChainContext.depthImageView = VK_NULL_HANDLE;
    }
    if (swapChainContext.depthImage != VK_NULL_HANDLE) {
        vkDestroyImage(device, swapChainContext.depthImage, nullptr);
        swapChainContext.depthImage = VK_NULL_HANDLE;
    }
    if (swapChainContext.depthImageMemory != VK_NULL_HANDLE) {
        vkFreeMemory(device, swapChainContext.depthImageMemory, nullptr);
        swapChainContext.depthImageMemory = VK_NULL_HANDLE;
    }
...
int main() {
    try {
        ...
        TextureContext texture = createTexture(
            physicalDevice,
            deviceContext,
            surface
        );
        ...
        destroyTexture(deviceContext.device, texture);
        destroyBuffer(deviceContext.device, indexBuffer);
